...
24. finding lr finder - one epoch to find the lr and changing the sched etc. - saved in onecycle/24lr.png and other losses and lrs in lrs/losses24lr.npy - max lr 
    should be 10^-0.23 = 0.588, init_lr = 0.058 (10 times smaller) epochs = 50, bs = 64 (didnt finish because the finding lr with max_lr up to 9 was too much for the 
    model and loss exploded to 190 during training, changed maximum_lr for find_lr.py to 1. to  maintain best loss 
25. changed maximum_lr to 1, same tests but with second run ish sigh 10^-1.64 = 0.0229 init_lr = 0.00229 TOO SLOW NOW TAKES 3 MINUTES FOR ONE EPOCH 
    but finished loss went up to 2 idk 
(for find_lr, last one would ideally be like this ETA: 0s - loss: 1.3939 - acc: 0.4957 - LRFinder: lr = 0.95185653)
26. try again, same as 25, with maximumlr in finding lr = 1. 10^-0.46 = 0.3467 , init_lr=0.02 ... its so slow though 3 minutes stil?? started training loss at 1.6
    umm not converging wellll - going to stop at 30 epochs, change max_lr to 0.6 and init_lr to 0.01, go to 30 more epochs
    ok yea not working im sorry this is not working not just going off and not going to converge at all (even though loss is decreasing - maybe might have to take a while for the accuracy to pass the threshold? but already took a long time 40+ epochs...) might not have worked due to to two times as high init lr and not as 
    large max_lr at the end to make the neccesary changes? a small init_lr is key i think 
27. going to try the 10^-0.23 = 0.588, init_lr = 0.01 because..... yea looking at the one cycle pngs most of the initialization lets the lr be around 0.6 for max_lr
    *****began with init_lr at 0.058 (consistently decreasing in loss but ac stayed the same), too big, decreased to 0.01 WITHOUT USING THE FIND_LR - (now starting to 
    converge much better, but much more jumpy compared to the 0.058) - ended with 50 epochs, valid acc around 0.96 - 0.97 - around 12 epochs was already at 0.7 valid
    acc [0.17025422269389742, 0.9702380952380952] yay best one (had 10 more epochs and lower lr by 0.02? might not be that big of a difference but futher training 
    time. training acc hit 1.00000 around last three epochs - 50 epochs might be the longest we can train without overfitting or screwing up model. 
28. trying to USE THE LR_FINDER actually instead of creating a new instance of the model using the specific lr - around -1.7 - around 0.04.. not that large of lr ..
    gets really bumpy at the end ... - going to max_lr = 0.04, with epoch = 70 init_lr = 0.01 (hm seems like bumpiness always around 0.6)
    [0.7279805172057379, 0.9434523809523809]
